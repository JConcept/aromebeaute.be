							...
							foreach( $portfolio_settings["items_name"] as $key => $item ){
								...
								if( "video" === $item ) {
									...
									if ( strpos($current_item,"vimeo") ) : #For Vimeo
										...
										echo "<li><iframe src='https://player.vimeo.com/video/{$url}' width='{$width}' height='{$height}' frameborder='0'></iframe></li>";
										
									elseif( strpos($current_item,"?v=") ): #For Youtube
										$url = substr( strrchr($current_item,"="),1);
										echo "<li><iframe src='https://www.youtube.com/embed/{$url}?wmode=opaque' width='{$width}' height='{$height}' frameborder='0'></iframe></li>";
									endif;
								} else {
