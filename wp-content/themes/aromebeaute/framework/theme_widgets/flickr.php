...
class MY_Flickr extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(false,IAMD_THEME_NAME.__(' Flickr Widget','spalab'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p><label for="<?php echo ( $this->get_field_id('flickr_id') ); ?>"><?php _e('Flickr ID:','spalab');?>
        ...
        <p><label for="<?php echo ( $this->get_field_id('count') ); ?>"><?php _e('How many entries do you want to show:','spalab');?>
         ...
         <p><label for="<?php echo ($this->get_field_id('show'));?>"><?php _e('What pictures to display','spalab'); ?>
         	<select class="widefat" id="<?php echo ( $this->get_field_id('show') ); ?>" name="<?php echo ( $this->get_field_name('show') ); ?>">
            <?php  $a = array("latest"=>__("Latest",'spalab'),"random"=>__("Random",'spalab'));
         ...
         <p><label for="<?php echo ( $this->get_field_id('size') );?>"><?php _e('What pictures to display','spalab'); ?>
         	<select class="widefat" id="<?php echo ( $this->get_field_id('size') ); ?>" name="<?php echo ( $this->get_field_name('size') ); ?>">
            <?php  $a = array("s"=>__("square",'spalab'),"t"=>__("thumbnail",'spalab'), "m" => __("medium",'spalab'));
			foreach($a as $key => $value ):
...
<?php
	...
	function widget($args, $instance) {
		...
		echo ( $before_widget );
			...
			src="https://www.flickr.com/badge_code_v2.gne?count='.$count.'&amp;display='.$show.'&amp;size='.$size.'&amp;layout=x&amp;source=user&amp;user='.$flickr_id.'"></script>'; 
			echo "</div>";
		echo ( $after_widget );
	}
}?>