...
class MY_Recent_Pages extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(false,IAMD_THEME_NAME.__(' Pages','spalab'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo esc_attr( $this->get_field_id('_pages') ); ?>">
			<?php _e('Choose the pages you want to display (multiple selection possible)','spalab');?></label>
            <select class="widefat" id="<?php echo ( $this->get_field_id('_pages').'[]' );?>" 
            	name="<?php echo ( $this->get_field_name('_pages').'[]' );?>" multiple="multiple">
                <option value=""><?php _e("Select Page",'spalab');?></option>
        ...
        <p><label for="<?php echo esc_attr( $this->get_field_id('_excerpt') ); ?>"><?php _e('Display title only or title &amp; excerpt','spalab');?></label>
        ...
        <p><input type="checkbox"  id="<?php echo esc_attr( $this->get_field_id('_enabled_image') );?>" name="<?php echo esc_attr( $this->get_field_name('_enabled_image') );?>"
	         <?php checked($_enabled_image); ?> /> <?php _e("Show Image",'spalab');?></p>  
        <!-- Form end-->
<?php
	}
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-pages-widget'><ul>";
			 ...
			 endwhile;
				...
				echo "<li><h6>".__('No Pages found','spalab')."</h6></li>";			 
			 endif;
			 wp_reset_query();
	 	echo "</ul></div>";			 
		echo ( $after_widget );
